...
# Load the data
data = pd.read_csv('CSV/AR.csv',on_bad_lines='skip')
...
data_dt = data[data['Author Keywords'].str.contains('augmented', na=False)]
...
keyword_counts = keyword_counts[keyword_counts.index != 'augmented reality']
...
plt.title('Top 10 Most Common Phrases Used with "Augmented Reality"')
plt.xlabel('Phrase')
plt.ylabel('Number of Occurrences')
plt.xticks(rotation=45, ha='right')
plt.tight_layout()
plt.show()
plt.savefig('Plots/AR_Phrases.png')
plt.close()
